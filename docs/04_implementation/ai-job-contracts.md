...
| `POST /candidate/interviews/{sessionId}/follow-up-question` | Candidate | answerId, previousQuestion, transcript, jobDescription 또는 documentSummary, profileContext(V1, server-built) | NCS 근거와 fact gate를 병렬 확인한다. 둘 중 하나라도 보완이 필요하면 동일 mode·답변시간의 private session question 하나만 세션 끝에 추가 |
