...
| Temporary provider placeholder allowlist | API Google/SMTP/OpenAI와 worker OpenAI의 명시된 key만 `<...>` 값을 warning으로 허용하고 missing/empty/그 외 placeholder는 실패 | 임시 계정 중단이 전체 배포를 막거나, 예상하지 않은 secret 미설정까지 조용히 통과 |
