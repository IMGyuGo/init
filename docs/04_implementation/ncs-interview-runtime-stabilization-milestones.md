...
1. 대표 브라우저 E2E는 NCS 공통 질문 6개와 개인화 질문 2개로 구성하고, 자동 회귀에서는 개인화 질문 0개 정책도 유지한다.
2. 개인화 질문이 필요한 지원서는 질문 2개가 `READY`가 되기 전 면접 시작을 차단하고 공통 질문으로 대체하지 않는다.
3. 세션 시작 전에 질문·binding·가중치 snapshot이 모두 존재하고 공통 6개 다음에 개인화 2개가 저장되는지 확인한다.
4. 첫 질문에 답변하고 다음 질문으로 이동한다.
5. 꼬리질문이 불필요하면 바로 두 번째 기본 질문을 표시한다.
6. 꼬리질문이 필요하면 같은 question mode의 질문을 한 번만 삽입한다.
7. worker를 중지하거나 timeout을 발생시켜도 다음 기본 질문으로 진행한다.
8. STT 실패 후 재답변을 한 번 허용하고, 다시 실패하면 점수 NULL 상태로 진행한다.
9. API를 재시작한 뒤에도 first-unanswered 질문과 질문 순서가 동일하다.
10. 기존 legacy snapshot을 NCS 세션으로 오인하지 않고 재생성 또는 명시적 차단한다.
11. 모든 질문 완료 후 리포트에 답변 ID, 질문 source, profile 점수 또는 평가 미완료 사유가 표시된다.
