...
- 고정 리포트는 커트라인 변경을 위해 다시 생성하지 않는다. API는 저장된 동일 총점·역량 점수를 공고의 자동판정 정책에 적용해 `PASS | HOLD | FAIL`을 동기 저장하고 면접관 그룹 목록에 먼저 노출한다. 면접관은 필요한 지원자만 검토 결과를 수정할 수 있고, 지원자는 공고 단위 API-012C 확정 이후에만 final decision과 알림을 조회한다.
