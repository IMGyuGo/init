...
| 지원자 관리 화면 | /company/recruitments/{recruitmentId}/applicants | list | 자동판정 그룹 지원자 목록 | 지원 상태·자동판정·검토 초안 조회 및 전형 상태 직접 수정 | 공고, 상태 필터, 검색어, PASS/HOLD/FAIL | effective decision 기준 그룹 표시와 리포트 완료·미확정 행의 드롭다운 저장 | GET /company/recruitments/{recruitmentId}/applicants / PATCH /company/applicants/{applicantId}/screening-review / PATCH /company/applicants/{applicantId}/screening-status | v1.0 | 자동 판정 snapshot은 보존하고 legacy 공고만 수동 상태 API 사용 |
| 지원자 관리 화면 | /company/recruitments/{recruitmentId}/applicants | button | 목표 합격자 선발 및 메일 전송 | 완료 리포트의 미확정 PASS/HOLD/FAIL을 점수순으로 PASS/FAIL 재배치 | targetPassCount | 상위 목표 인원은 합격, 나머지는 불합격 처리 후 최종 합격자에게만 메일 발송 | POST /company/recruitments/{recruitmentId}/applicants/pass-mails | v1.0 | HOLD 포함, 이미 발송된 합격 메일은 제외 |
...
| 지원자 평가 상세 화면 | /company/applicants/{applicantId}/evaluation | form | 자동판정 검토/수정 | 자동판정 유지 또는 PASS/HOLD/FAIL 검토 초안 수정 | 검토 결과, 변경 사유 | effective decision 저장 후 목록 그룹 갱신 | PATCH /company/applicants/{applicantId}/screening-review | v1.0 | 자동판정과 다르면 변경 사유 필수, 확정 후 수정 불가 |
...
| 채용 AI 면접 결과 화면 | /candidate/applications/{applicationId}/report | page | 채용 AI 면접 결과 | 지원자용 제한 결과 조회 | 지원 ID, 면접 세션 ID | 면접관 확정 전 결과 검토 중, 확정 후 제한된 기업 전형 결정 표시 | GET /candidate/applications/{applicationId}/report | v1.0 | reportType=RECRUITING_REPORT. 면접관 확정 후에만 공개하고 내부 메모·평가 상세는 제외 |
| 채용 AI 면접 결과 화면 | /candidate/applications/{applicationId}/report | section | 전형 상태 표시 | 채용 전형 진행 상태 조회 | 지원 ID | 면접관 확정 전 결과 검토 중, 확정 후 저장된 기업 전형 결정 표시 | GET /candidate/applications/{applicationId}/status | v1.0 | resultPublicationStatus=PENDING이면 내부 판정 비노출. 기업용 내부 메모·수동 평가 상세는 노출하지 않음 |
