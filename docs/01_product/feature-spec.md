...
| 지원자 관리 화면 | /company/recruitments/{recruitmentId}/applicants | list | 자동판정 그룹 지원자 목록 | 지원 상태·자동판정·검토 초안 조회 | 공고, 상태 필터, 검색어 | effective decision 기준 합격/보류/불합격 그룹과 재처리 필요 목록 표시 | GET /company/recruitments/{recruitmentId}/applicants | v1.0 | 면접관이 전원을 체크하지 않고 필요한 지원자만 상세 검토 |
| 지원자 관리 화면 | /company/recruitments/{recruitmentId}/applicants | button | 전형 결과 일괄 확정 | 현재 판정 가능 미확정 결과 일괄 확정 및 알림 등록 | expectedEligibleCount | Alert 재확인 후 최종 결과 확정 | POST /company/recruitments/{recruitmentId}/screening-results/confirm | v1.0 | PASS/HOLD/FAIL 및 제외 인원을 표시하고 두 번째 확정에서만 요청 |
