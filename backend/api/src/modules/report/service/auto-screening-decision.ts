...
export type AutoScreeningReasonCode =
...
export interface AutoScreeningDecisionInput {
  ...
  criteria: Array<{ active: boolean; score: number | null; passScore: number | null; evaluationComplete: boolean }>;
}
...
export function decideAutoScreening(input: AutoScreeningDecisionInput): {
  ...
  reasonCode: AutoScreeningReasonCode | null;
} {
  ...
  if (input.report.status === "FAILED") return { decision: "RETRY", reasonCode: "RETRY_REPORT_FAILED" };
  if (input.hasTerminalSttUnavailable) return { decision: "RETRY", reasonCode: "RETRY_STT_UNAVAILABLE" };
  ...
  if (
    ...
    activeCriteria.some((criterion) => !isValidScore(criterion.score) || !isValidScore(criterion.passScore))
  ) {
