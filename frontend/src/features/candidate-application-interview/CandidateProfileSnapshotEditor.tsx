...
export function CandidateProfileSnapshotEditor({ value, onChange, emailReadOnly = false }: Props) {
    ...
    <div className="candidate-profile-snapshot-editor">
      <fieldset className="candidate-profile-snapshot-basic">
        ...
        <div className="candidate-profile-grid">
          <EditorField label="이름" required><input value={value.name} maxLength={100} onChange={(event) => patch({ name: event.currentTarget.value })} /></EditorField>
      ...
      <SnapshotSection title="학력" section="educations" count={value.educations.length} open={open.educations} onToggle={() => toggle("educations")} onAdd={() => add("educations")}>
        {value.educations.map((item, index) => <SnapshotItem key={`education-${index}`} label={`학력 ${index + 1}`} onRemove={() => remove("educations", index)}>
          ...
          <EditorField label="입학년월"><input type="month" value={item.startMonth} onChange={(event) => replace("educations", index, { ...item, startMonth: event.currentTarget.value })} /></EditorField>
          <EditorField label="졸업·예정년월"><input type="month" disabled={item.status === "ENROLLED" || item.status === "LEAVE_OF_ABSENCE"} value={item.endMonth ?? ""} onChange={(event) => replace("educations", index, { ...item, endMonth: nullable(event.currentTarget.value) })} /></EditorField>
        </SnapshotItem>)}
      </SnapshotSection>
      ...
      <SnapshotSection title="경력" section="careers" count={value.careers.length} open={open.careers} onToggle={() => toggle("careers")} onAdd={() => add("careers")}>
        {value.careers.map((item, index) => <SnapshotItem key={`career-${index}`} label={`경력 ${index + 1}`} onRemove={() => remove("careers", index)}>
          ...
          <EditorField label="입사년월"><input type="month" value={item.startMonth} onChange={(event) => replace("careers", index, { ...item, startMonth: event.currentTarget.value })} /></EditorField>
          <EditorField label="퇴사년월"><input type="month" disabled={item.isCurrent} value={item.endMonth ?? ""} onChange={(event) => replace("careers", index, { ...item, endMonth: nullable(event.currentTarget.value) })} /></EditorField>
      ...
      <SnapshotSection title="프로젝트·경험·활동·교육" section="activities" count={value.activities.length} open={open.activities} onToggle={() => toggle("activities")} onAdd={() => add("activities")}>
        {value.activities.map((item, index) => <SnapshotItem key={`activity-${index}`} label={`활동 ${index + 1}`} onRemove={() => remove("activities", index)}>
          ...
          <EditorField label="시작일"><input type="date" value={item.startDate} onChange={(event) => replace("activities", index, { ...item, startDate: event.currentTarget.value })} /></EditorField>
          <EditorField label="종료일"><input type="date" disabled={item.isOngoing} value={item.endDate ?? ""} onChange={(event) => replace("activities", index, { ...item, endDate: nullable(event.currentTarget.value) })} /></EditorField>
      ...
      <SnapshotSection title="자격·어학·수상" section="credentials" count={value.credentials.length} open={open.credentials} onToggle={() => toggle("credentials")} onAdd={() => add("credentials")}>
        {value.credentials.map((item, index) => <SnapshotItem key={`credential-${index}`} label={`자격·어학·수상 ${index + 1}`} onRemove={() => remove("credentials", index)}>
          ...
          <EditorField label="취득년월"><input type="month" value={item.acquiredMonth} onChange={(event) => replace("credentials", index, { ...item, acquiredMonth: event.currentTarget.value })} /></EditorField>
...
function EditorField({ label, required = false, wide = false, children }: { label: string; required?: boolean; wide?: boolean; children: ReactNode }) {
  return <label className={wide ? "candidate-profile-field candidate-profile-field--wide" : "candidate-profile-field"}><span>{label}{required ? <span className="req-mark"> *</span> : null}</span>{children}</label>;
}
